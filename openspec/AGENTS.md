...
- 搭建结构：`proposal.md`、`tasks.md`、`design.md`（仅在需要时），以及每个受影响能力的规范变更（delta specs）
- 编写变更：使用 `## ADDED|MODIFIED|REMOVED|RENAMED Requirements`；每个需求至少包含一个 `#### Scenario:`
...
2. 选择一个唯一的动词开头的 `change-id`，并在 `openspec/changes/<id>/` 下搭建 `proposal.md`、`tasks.md`、可选的 `design.md` 和规范变更。
3. 使用 `## ADDED|MODIFIED|REMOVED Requirements` 起草规范变更，每个需求至少包含一个 `#### Scenario:`。
...
3. **创建规范变更：** `specs/[capability]/spec.md`
...
如果多个能力受到影响，在 `changes/[change-id]/specs/<capability>/spec.md` 下创建多个变更文件——每个能力一个。
...
- ADDED：引入可以独立作为需求的新能力或子能力。当变更与现有需求正交时（例如，添加"斜杠命令配置"）优先使用 ADDED，而不是改变现有需求的语义。
- MODIFIED：更改现有行为、范围或验收标准。始终粘贴完整的、更新后的需求内容（标题 + 所有场景）。归档器将用您在此处提供的内容替换整个需求；部分变更将丢失之前的详细信息。
